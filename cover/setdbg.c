...
char _Covpp_Copyright_[] = "Copyright (C) 1990-1993 by Jarmo Ruuth";
...
static FILE* src_in;
static unsigned long file_pos = 0;
 ...
 *	cover_end_function
 */
static void cover_end_function()
{
	char buf[40];

        if (do_addinout){
	    sprintf(buf, "printf(\"out:%s\\n\"\);",function);
	    yyout_fputs(buf);
        } else {
...
static void finish_file(void)
{
	...
	if (exitcode != 0) {
	    ...
	    yyout_fprintf(
	        ...
	        "{\n"
	        "  if (!_cover_initp_){\n"
	        "    _cover_init_file_(\"%s\",\"%s\",&_cover_tab_,sizeof(_cover_t),%d);\n"
...
static char* get_nthline(long line_no)
{
        ...
        if (line_no >= file_pos) {
            ...
            /* rewind file */
            src_in = freopen(src_name,"r",src_in);
	...
	if (argc - i != 3) {
		printf("Usage: covpp [options] <infile> <outfile> <listfile>\n"
		       ...
		       "         -v verbose\n"
                   ...
                   "         -z add in and out debug texts to functions\n"
